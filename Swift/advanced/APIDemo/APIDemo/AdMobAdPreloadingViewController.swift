  ...
  FullScreenContentDelegate
{
  ...
  private func configurePreloadViews() {
    guard
      let interstitialView = PreloadView.load(
        title: "Interstitial",
        showDelegate: { [weak self] in
          self?.showInterstitialAd()
        }),
      let rewardedView = PreloadView.load(
        title: "Rewarded",
        showDelegate: { [weak self] in
          self?.showRewardedAd()
        }),
      let appOpenView = PreloadView.load(
        title: "App open",
        showDelegate: { [weak self] in
          self?.showAppOpenAd()
        }),
      let rewardedInterstitialView = PreloadView.load(
    ...
    self.rewardedView = rewardedView
    preloadContainer.addArrangedSubview(rewardedView)

    self.appOpenView = appOpenView
    preloadContainer.addArrangedSubview(appOpenView)

    self.rewardedInterstitialView = rewardedInterstitialView
  ...
  private func updatePreloadView(preloadView: PreloadView?, isAvailable: Bool) {
    guard let preloadView else { return }
