...
class AdManagerSCARSnippets: NSObject {
  ...
  func loadNative(adUnitID: String) async {
    ...
    do {
      ...
      let response = await fetchSignalResponse(signalRequest: signalResponse)
  ...
  func loadNativePlusBanner(adUnitID: String) async {
    ...
    // Refer to the AdSize class for available ad sizes.
    signalRequest.adSizes = [nsValue(for: currentOrientationInlineAdaptiveBanner(width: 375))]
  ...
  func fetchSignalResponse(signalRequest: Signal) async -> String {
...
extension AdManagerSCARSnippets: NativeAdLoaderDelegate {
  ...
  func validAdSizes(for adLoader: AdLoader) -> [NSValue] {
