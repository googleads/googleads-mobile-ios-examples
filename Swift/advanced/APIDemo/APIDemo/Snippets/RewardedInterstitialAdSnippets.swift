...
private class RewardedInterstitialAdSnippets: UIViewController, FullScreenContentDelegate {
  ...
  private let publisherAdUnitID = "/21775744923/example/rewarded-interstitial"

  // [START load_ad]
  func loadRewardedIntertitialAd() async {
    do {
  ...
  // [START load_ad_gam]
  func loadGAMRewardedInterstitialAd() async {
    do {
      rewardedInterstitialAd = try await RewardedInterstitialAd.load(
        // Replace this ad unit ID with your own ad unit ID.
        with: publisherAdUnitID, request: AdManagerRequest())
      // [START set_delegate]
      rewardedInterstitialAd?.fullScreenContentDelegate = self
      // [END set_delegate]
    } catch {
  ...
  private func validateGAMServerSideVerification() async {
    do {
      rewardedInterstitialAd = try await RewardedInterstitialAd.load(
        // Replace this ad unit ID with your own ad unit ID.
        with: publisherAdUnitID, request: AdManagerRequest())
